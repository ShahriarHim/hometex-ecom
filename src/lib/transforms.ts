...
export const transformAPIProductToProduct = (
  data: APIProduct | { product: APIProduct } | unknown
): Product => {
  // Handle case where we receive the wrapper object or invalid data
  if (!data || typeof data !== "object") {
    console.error("transformAPIProductToProduct received invalid data:", data);
    return {
      id: "0",
  ...
  // Auto-unwrap if nested property 'product' exists and looks like a product
  const apiProduct = (
    "product" in data &&
  ...
  const id = apiProduct.id !== undefined && apiProduct.id !== null ? apiProduct.id.toString() : "0";
  ...
  // Get images - priority: media.primary_image > primary_photo > images array > media gallery > thumbnail > brand logo
  const images: string[] = [];

  // Helper to extract URL from image object or string
  const extractImageUrl = (
    img: string | { url?: string; image_url?: string; path?: string } | null | undefined
  ): string | null => {
    if (!img) {
      return null;
    }
  ...
  // Priority 1: media.primary_image
  const primaryImageUrl = apiProduct.media?.primary_image
    ? extractImageUrl(apiProduct.media.primary_image)
    : null;
  if (primaryImageUrl) {
    images.push(primaryImageUrl);
  } else if (apiProduct.primary_photo) {
    // Priority 2: primary_photo (legacy field)
    ...
    // Priority 5: thumbnail
    const thumbnailUrl = apiProduct.media?.thumbnail
      ? extractImageUrl(apiProduct.media.thumbnail)
      : apiProduct.thumbnail;
    if (thumbnailUrl) {
      images.push(thumbnailUrl);
    } else if (apiProduct.brand?.logo) {
      // Priority 6: brand logo (fallback)
  ...
  return {
    ...
    related_products: apiProduct.related_products
      ? {
          similar_products: apiProduct.related_products.similar_products,
          frequently_bought_together: apiProduct.related_products.frequently_bought_together,
          customers_also_viewed: apiProduct.related_products.customers_also_viewed,
          recently_viewed: apiProduct.related_products.recently_viewed,
        }
      : undefined,
  };
};
