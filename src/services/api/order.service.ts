...
export const orderService = {
   ...
   */
  createGuestOrder: async (orderData: GuestOrderRequest): Promise<GuestOrderResponse> => {
    const response = await fetchWithFallback("/api/guest/checkout", env.apiBaseUrl, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Accept: "application/json",
    ...
    return handleApiResponse<GuestOrderResponse>(response);
  },

  /**
   * Track guest order
   */
  trackGuestOrder: async (orderNumber: string, email: string): Promise<GuestOrderResponse> => {
    const response = await fetchWithFallback(
      `/api/guest/orders/track?orderNumber=${encodeURIComponent(orderNumber)}&email=${encodeURIComponent(email)}`,
      env.apiBaseUrl,
      {
        method: "GET",
        headers: {
