...
const ProductDetailNew = () => {
  ...
  const rawData = productResponse?.data;

  // Debug log to inspect structure
  useEffect(() => {
    if (rawData) {
      console.warn("ProductDetail: Raw API Data:", rawData);
    }
  }, [rawData]);

  // Handle case where API returns nested { product: ... } structure
  const product = (
    rawData && typeof rawData === "object" && "product" in rawData
      ? (rawData as unknown as { product: DetailedProduct }).product
      : rawData
  ) as DetailedProduct | undefined;
        ...
        <main className="flex-1 container mx-auto px-4 py-6">
          ...
          <div className="grid lg:grid-cols-2 gap-12 mb-12">
            ...
            <div>
              <MediaGallery
                ...
                productName={product?.name ?? ""}
                isBlurred={isPopupVisible}
                overlay={
                  product && (
                    <ProductPopularityPopup
                      productId={product.id}
          ...
          <Tabs defaultValue="details" className="mb-12">
            <TabsList className="grid w-full grid-cols-4 mb-8">
              <TabsTrigger value="details" className="group flex items-center gap-2">
                <FileText className="h-5 w-5 text-yellow-600" />
                Details
              </TabsTrigger>
              <TabsTrigger value="specs" className="group flex items-center gap-2">
                <List className="h-5 w-5 text-yellow-600" />
                Specifications
              </TabsTrigger>
              <TabsTrigger value="reviews" className="group flex items-center gap-2">
                <MessageSquare className="h-5 w-5 text-yellow-600" />
                Reviews ({reviews?.review_count ?? 0})
              </TabsTrigger>
              <TabsTrigger value="shipping" className="group flex items-center gap-2">
            ...
            {similarProductsLoading ? (
              ...
              >
                {similarProducts.map((similarProduct) => {
                  ...
                  // Extract image URL from media.primary_image or fallback to primary_photo
                  ...
                  const primaryImageUrl = productAny.media?.primary_image
                    ? extractImageUrl(productAny.media.primary_image)
                    : null;
