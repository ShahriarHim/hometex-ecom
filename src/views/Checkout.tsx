...
import { ApiError, locationService, orderService, userService } from "@/services/api";
...
const Checkout = () => {
  ...
  }, [isAuthenticated, isGuest, items.length, orderConfirmed, router]);
  ...
  const [confirmedOrderNumber, setConfirmedOrderNumber] = useState<string | undefined>(undefined);
  const [confirmedEmail, setConfirmedEmail] = useState<string | undefined>(undefined);

  if (orderConfirmed) {
    return (
      <OrderConfirmation
        orderId={confirmedOrderId} // kept for logic compatibility but likely not shown
  ...
  if ((!isAuthenticated && !isGuest) || items.length === 0) {
    return null;
  }
  ...
  const handleCheckout = async (e: React.FormEvent) => {
    ...
    if (isGuest) {
      try {
        const guestOrderData = {
          ...
          paymentMethod: {
            type: paymentMethod === "cod" ? "cod" : "ssl_commerce",
          },
          notes: giftMessage || undefined,
          billingSameAsShipping: true,
        };

        const response = await orderService.createGuestOrder(guestOrderData);

        if (response.success) {
          isNavigatingRef.current = true;
          setConfirmedOrderId(String(response.data.order.id));
          setConfirmedOrderNumber(response.data.order.orderNumber);
          setConfirmedEmail(response.data.customer.email);
          setOrderConfirmed(true);
          clearCart();
          toast.success("Guest order placed successfully!");
        } else {
          toast.error(response.message || "Failed to place guest order.");
        }
      } catch (error) {
        console.error("Guest checkout error:", error);
        toast.error("Failed to place guest order. Please try again.");
      } finally {
        setLoading(false);
      }
      return;
    ...
    try {
      ...
      setConfirmedEmail(order.shippingAddress.email);
      setOrderConfirmed(true);
      clearCart();
    } catch (error) {
